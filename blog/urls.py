...
urlpatterns = patterns('blog.views',
    ...
    (r'^(?P<year>\d{4})/(?P<month>\d{2})/$', 'post_archive'),
    ...
    (r'^categories/(?P<category_slug>[-\w]+)/(?P<year>\d{4})/(?P<month>\d{2})/$', 'post_index'),
    (r'^categories/(?P<slug>[-\w]+)/$', 'category_detail'),
    (r'^series/$', 'series_index'),
    (r'^series/(?P<series_slug>[-\w]+)/(?P<year>\d{4})/$', 'post_index'),
    (r'^series/(?P<series_slug>[-\w]+)/(?P<year>\d{4})/(?P<month>\d{2})/$', 'post_index'),
    (r'^series/(?P<slug>[-\w]+)/$', 'series_detail'),
    (r'^tags/$', 'tag_index'),
    (r'^tags/(?P<tag>[-\w]+)/(?P<year>\d{4})/$', 'post_index'),
    (r'^tags/(?P<tag>[-\w]+)/(?P<year>\d{4})/(?P<month>\d{2})/$', 'post_index'),
    (r'^tags/(?P<tag>.+)/$', 'tag_detail'),
    (r'^(?P<year>\d{4})/(?P<month>\d{2})/(?P<day>\d{2})/(?P<slug>[-\w]+)/$', 'post_detail'),
)
